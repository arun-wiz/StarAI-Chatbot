...
# Required env:
# AWS_EKS_CLUSTER_NAME, AWS_EKS_REGION
# AWS_ECR_IMAGE_TAGGED
# Optional legacy aliases remain supported:
# EKS_CLUSTER_NAME, EKS_CLUSTER_NAME_B, EKS_REGION, EKS_REGION_B, ECR_IMAGE_TAGGED
...
AWS_ECR_IMAGE_TAGGED="${AWS_ECR_IMAGE_TAGGED:-${ECR_IMAGE_TAGGED:-}}"
AWS_PUBLIC_DOMAIN="${AWS_PUBLIC_DOMAIN:-${PUBLIC_DOMAIN:-}}"
AWS_ALB_ACM_ARN="${AWS_ALB_ACM_ARN:-${ALB_ACM_ARN:-}}"
DEMO_MODE="${DEMO_MODE:-false}"
LANGFLOW_SEED_IMAGE="${LANGFLOW_SEED_IMAGE:-}"

: "${AWS_EKS_CLUSTER_NAME:?missing}"
: "${AWS_EKS_REGION:?missing}"
: "${AWS_ECR_IMAGE_TAGGED:?missing}"

echo "[INFO] Setting kubeconfig for ${AWS_EKS_CLUSTER_NAME} (${AWS_EKS_REGION})..."
...
if aws eks describe-cluster --name "${AWS_EKS_CLUSTER_NAME}" --region "${AWS_EKS_REGION}" --query 'cluster.computeConfig.enabled' --output text >/dev/null 2>&1; then
  compute_enabled="$(aws eks describe-cluster --name "${AWS_EKS_CLUSTER_NAME}" --region "${AWS_EKS_REGION}" --query 'cluster.computeConfig.enabled' --output text 2>/dev/null || true)"
...
DEPLOY_FILE="manifests/deployment-noseed.yaml"
...
if [[ "${DEMO_MODE}" != "true" ]]; then
  if [[ -n "${AWS_PUBLIC_DOMAIN}" ]]; then
    sed -i "s#your-domain.example.com#${AWS_PUBLIC_DOMAIN}#g" "${ING_FILE_CUSTOM}"
  fi
  if [[ -n "${AWS_ALB_ACM_ARN}" ]]; then
    sed -i "s#arn:aws:acm:us-east-1:879381248241:certificate/REPLACE_ME#${AWS_ALB_ACM_ARN}#g" "${ING_FILE_CUSTOM}"
  fi
fi
